...
/**
 * Extract features from an analysed document. `findings` are the rule hits (for
 * the hollowness-rate features); `ctx` carries the parse.
 */
export function docFeatures(ctx: Context, findings: Finding[]): DocFeatures {
  const deprel: string[] = [];
  const pos: string[] = [];
  ...
  // hollowness rule rates (per 100 words), by rule name
  const ruleCount = new Map<string, number>();
  const catCount = new Map<string, number>();
  for (const f of findings) {
    ruleCount.set(f.rule, (ruleCount.get(f.rule) ?? 0) + 1);
  ...
  const scalars: Record<string, number> = {
    ...
    r_copulaAvoid: per100(ruleCount.get('copula-avoid') ?? 0),
    r_vagueAttr: per100(ruleCount.get('vague-attribution') ?? 0),
    r_participial: per100(ruleCount.get('participial-appendage') ?? 0),
    r_negParallel: per100(ruleCount.get('neg-parallel') ?? 0),
    r_triad: per100(ruleCount.get('triad') ?? 0),
