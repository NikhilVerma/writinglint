 ...
 * (eval/data/, third-party text kept private). If the data dir is absent, the
 * harness degrades gracefully with a pointer to eval/data/README.md.
 *
...
import { analyze } from '../src/detector/analyze.js';
import { loadParser } from '../src/detector/parser-node.js';
import type { Model } from '../src/detector/classifier.js';
import type { Analysis, Category } from '../src/detector/types.js';

const MODEL_PATH = fileURLToPath(new URL('../models/classifier.json', import.meta.url));
...
export async function score(docs: Doc[]): Promise<Scored[]> {
  ...
  const out: Scored[] = [];
  for (const d of docs) out.push({ ...d, analysis: await analyze(d.text, parser, model) });
  return out;
}
...
export function confusionAt(human: Scored[], ai: Scored[], threshold: number): Confusion {
  const fp = human.filter((d) => d.analysis.stats.score >= threshold).length;
  const tn = human.length - fp;
  const tp = ai.filter((d) => d.analysis.stats.score >= threshold).length;
...
export function auc(human: Scored[], ai: Scored[]): number {
  ...
  for (const a of ai)
    for (const h of human) {
      const da = a.analysis.stats.score;
      const dh = h.analysis.stats.score;
      wins += da > dh ? 1 : da === dh ? 0.5 : 0;
    }
  return wins / (human.length * ai.length);
}

/** Aggregate per-rule firing counts across a set of scored docs. */
export function ruleCounts(docs: Scored[]): Map<string, number> {
  const m = new Map<string, number>();
  for (const d of docs)
    for (const f of d.analysis.findings) m.set(f.rule, (m.get(f.rule) ?? 0) + 1);
  return m;
}

/** Aggregate per-category firing counts. */
export function categoryCounts(docs: Scored[]): Map<Category, number> {
  const m = new Map<Category, number>();
  for (const d of docs)
    for (const f of d.analysis.findings) m.set(f.category, (m.get(f.category) ?? 0) + 1);
  return m;
}
