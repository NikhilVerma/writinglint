...
function scoreLine(label: string, docs: Scored[]): void {
  ...
  const scores = docs.map((d) => d.analysis.stats.score);
  console.log(
...
function reportSplit(
  ...
  const humanMean = mean(human.map((d) => d.analysis.stats.score));
  ...
  if (blind) {
    ...
    const fpDocs = human.filter((d) => d.analysis.stats.score >= threshold);
    if (fpDocs.length) {
      console.log(`\n  ⚠ Human docs FLAGGED as AI (false positives):`);
      for (const d of fpDocs)
        console.log(`      ${d.analysis.stats.score.toString().padStart(3)}  ${d.id}`);
    }
    const fnDocs = ai.filter((d) => d.analysis.stats.score < threshold);
    if (fnDocs.length) {
