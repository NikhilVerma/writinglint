...
import { analyzeContext } from '../src/detector/analyze.js';
import { docFeatures, type DocFeatures } from '../src/detector/features.js';
import { predict, trainModel, type Model } from '../src/detector/classifier.js';
...
async function featurize(rows: Row[]) {
  ...
  for (const { doc, label } of rows) {
    const { ctx, findings } = await analyzeContext(doc.text, parser);
    f.push(docFeatures(ctx, findings));
    y.push(label);
  }
  return { f, y };
}
...
console.log(`\nSaved model (${model.vocab.length} ngram + ${model.scalarNames.length} scalar) → models/classifier.json  [threshold ${cvBest.thr.toFixed(2)}]`);
