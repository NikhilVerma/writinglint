...
import { bundledModelDirectory } from 'writinglint-parser-node/onnx';
...
/** Resolve the bundled/explicit model or populate the emergency versioned cache. */
export async function ensureModel(options: ModelOptions = {}): Promise<string> {
  const candidates = [
    options.explicit,
    process.env.SLOPSIFT_MODEL,
  ...
  const directory = join(cacheRoot, 'slopsift', 'models', VERSION);
  ...
  const base = (process.env.SLOPSIFT_MODEL_BASE_URL ?? DEFAULT_BASE).replace(/\/$/, '');
  options.onProgress?.(`Downloading SlopSift model ${VERSION} (~16 MB, once)…`);
  try {
