...
import { createSloplint, inputKind, type MinimumLevel, type Sloplint } from 'sloplint';
import { lintPath } from './documents.js';

const CONFIGURATION = 'sloplint';
const SOURCE = 'sloplint';
...
function settings(document?: vscode.TextDocument): Settings {
  ...
  return {
    ...
    downloadModel: config.get<boolean>('downloadModel', true),
    debounce: config.get<number>('debounceMilliseconds', 450),
  };
}

class SloplintController implements vscode.Disposable {
  private readonly diagnostics = vscode.languages.createDiagnosticCollection(SOURCE);
  private readonly output = vscode.window.createOutputChannel('Sloplint', { log: true });
  private readonly disposables: vscode.Disposable[] = [];
  private readonly timers = new Map<string, NodeJS.Timeout>();
  private readonly generations = new Map<string, number>();
  private engine?: { key: string; promise: Promise<Sloplint> };
  ...
  constructor() {
    this.disposables.push(
      ...
      vscode.commands.registerCommand('sloplint.lintDocument', async () => {
        const document = vscode.window.activeTextEditor?.document;
        if (!document) {
          void vscode.window.showInformationMessage('Sloplint: no active text document.');
          return;
        }
        await vscode.window.withProgress(
          { location: vscode.ProgressLocation.Notification, title: 'Sloplint: linting document' },
          () => this.lint(document, true),
        );
      }),
      vscode.commands.registerCommand('sloplint.showOutput', () => this.output.show(true)),
    );
  ...
  private getEngine(config: Settings): Promise<Sloplint> {
    const key = `${config.modelPath ?? '<cache>'}:${config.downloadModel}`;
    if (this.engine?.key === key) return this.engine.promise;
    const promise = createSloplint({
      explicit: config.modelPath,
  ...
  async lint(document: vscode.TextDocument, explicit = false): Promise<void> {
    ...
    if (!config.enabled || !inputKind(path)) {
      ...
      if (explicit && config.enabled) {
        void vscode.window.showInformationMessage(`Sloplint does not support ${document.languageId} documents yet.`);
      }
      return;
    }
    ...
    try {
      ...
      if (explicit) {
        void vscode.window.showInformationMessage(`Sloplint found ${mapped.length} issue${mapped.length === 1 ? '' : 's'}.`);
      }
    } catch (error) {
      ...
      if (explicit || this.lastAutomaticError !== message) {
        ...
        const action = await vscode.window.showWarningMessage(`Sloplint could not lint this document: ${message}`, 'Show Output');
...
export function activate(context: vscode.ExtensionContext): void {
  context.subscriptions.push(new SloplintController());
}
