...
An early Manifest V3 extension that puts Sloplint diagnostics into web editors.
...
npm run check-types -w sloplint-chrome-extension
npm run smoke -w sloplint-chrome-extension
```

Open `chrome://extensions`, enable Developer mode, choose **Load unpacked**, and
select `packages/sloplint-chrome/dist`.

For a small manual harness, serve `packages/sloplint-chrome/test` over HTTP and
open `manual.html`. Focus or edit the textarea; the field badge should appear
after the model's first cold load.

On a clean checkout, `npm run build` downloads the pinned `compact-int8-v1`
model into the ignored `models/rule-family-50-onnx-int8` directory and verifies
every file against a committed SHA-256 checksum before packaging it. Set
`SLOPLINT_MODEL` to use another complete local INT8 bundle. Use `npm run
build:offline` when the pinned model is already present and network access must
be forbidden. The approximately 15 MB parser plus the ONNX WASM runtime are
copied into the extension, so the built extension works offline.

Data handling is documented in [PRIVACY.md](./PRIVACY.md). Host the matching
policy page from `packages/sloplint-web` before a Chrome Web Store submission.

CI that should not download the model can still run the complete source gate:

```sh
npm run check -w sloplint-chrome-extension
```
