...
const model = process.env.SLOPLINT_MODEL ?? join(repo, 'models/rule-family-50-onnx-int8');
const requiredModelFiles = ['manifest.json', 'parser.onnx', 'relations.onnx', 'tokenizer/tokenizer.json'];

for (const file of requiredModelFiles) {
  if (!existsSync(join(model, file))) {
    throw new Error(`Missing ${join(model, file)}. Set SLOPLINT_MODEL to an INT8 model bundle.`);
  }
}
await verifyModel(model, {
  pinnedManifest: process.env.SLOPLINT_MODEL ? undefined : PINNED_MANIFEST_SHA256,
});
