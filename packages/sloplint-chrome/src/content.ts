...
shadow.innerHTML = `
  ...
  <button class="badge" type="button" aria-label="Open Sloplint diagnostics"></button>
  <section class="panel" role="dialog" aria-label="Sloplint diagnostics"><div class="head"><span class="wordmark">sloplint</span><span class="count"></span></div><div class="list"></div></section>`;
...
function isEditable(target: EventTarget | null): target is Editable {
  if (!(target instanceof HTMLElement) || target.closest('[data-sloplint="ui"]')) return false;
...
function decorateControl(state: FieldState, element: HTMLInputElement | HTMLTextAreaElement): void {
  ...
  mirror.dataset.sloplint = 'mirror';
...
function decorateContentEditable(state: FieldState, element: HTMLElement): void {
  ...
  const name = `sloplint-${nextHighlight++}`;
  highlights.set(name, new HighlightCtor(...ranges));
  const style = document.createElement('style');
  style.dataset.sloplintHighlight = name;
...
function renderPanel(): void {
  ...
  badge.setAttribute(
    'aria-label',
    active.lints.length ? `Open ${active.lints.length} Sloplint diagnostics` : 'No Sloplint diagnostics',
  );
