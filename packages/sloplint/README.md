...
```bash
npm run sloplint -- .
npm run sloplint -- "docs/**/*.md" "src/**/*.{ts,tsx}"
npm run sloplint -- . --format json
npm run sloplint -- . --level info
```

The package is intentionally marked private while the exact unscoped
`sloplint` name is under npm review. CI still installs the packed CLI into an
isolated non-workspace project. Restore public publishing only after the final
package identity is approved and the registry smoke test can install it.
...
against the pinned release manifest before inference. Set `SLOPLINT_MODEL` or
