...
- npm delivery: bundled once in `writinglint-parser-node/model`; WritingLint and
  Sloplint consume that dependency without duplicating the weights
- editor delivery: copied into Chrome and platform-specific VS Code artifacts
- browser demo delivery: `https://models.sloplint.dev/compact-int8-v1/*`, backed
  by the `sloplint-models` R2 bucket
- fallback cache: `$XDG_CACHE_HOME/sloplint/models/compact-int8-v1`, used only
...
9. Upload the immutable browser artifacts with
   `scripts/upload-sloplint-assets-to-r2.sh`.
10. Bump the version and pinned manifest SHA-256 in Sloplint's `src/model.ts`,
