...
Usage:
  sloplint [patterns...]              Lint files, directories, or globs
  bunx sloplint .                    Lint the current project
...
async function run(): Promise<void> {
  ...
  catch (error) { console.error(`sloplint: ${(error as Error).message}\nRun sloplint --help for usage.`); process.exitCode = 2; return; }
  ...
  try {
    ...
    console.error(`sloplint: ${(error as Error).message}`);
    process.exitCode = 2;
  }
}
