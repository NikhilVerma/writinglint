...
const pendingPublication = new Set(['sloplint']);
...
for (const name of packagedProjects) {
  ...
  if (pendingPublication.has(name)) {
    if (!manifest.private) fail(`${name} must remain private until its npm package identity is approved`);
  } else if (manifest.private) fail(`${name} is unexpectedly private`);
...
for (const [name, file] of [
  ...
  ['sloplint', 'packages/sloplint/dist/cli.js'],
]) {
