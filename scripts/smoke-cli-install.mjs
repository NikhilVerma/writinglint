...
const packageName = mode === 'published-writinglint' ? 'writinglint' : 'slopsift';
const packageDirectory = packageName === 'writinglint' ? 'cli' : 'slopsift';
const source = join(root, `examples/${packageName}-npm-smoke`);
const temporary = await mkdtemp(join(tmpdir(), `slopsift-${mode}-`));
const consumer = join(temporary, 'consumer');
const npmEnvironment = {
  ...process.env,
  npm_config_cache: join(temporary, 'npm-cache'),
};
const consumerNpmEnvironment = {
  ...npmEnvironment,
