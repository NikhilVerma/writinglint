...
from data import PairedDependencyDataset, UPOS, collate, collate_paired
...
def main() -> None:
    ...
    parser.add_argument("--rule-repeat", type=int, default=10)
    ...
    teacher_config = json.loads((args.teacher / "config.json").read_text())
    ...
    def paired(split: str) -> PairedDependencyDataset:
        return PairedDependencyDataset(
            find_split(args.data_dir, split), student_tokenizer, teacher_tokenizer,
            relation_to_id, args.max_subwords,
        )
    ...
    train_loader = DataLoader(
        combined_train, batch_size=args.batch_size, shuffle=True, collate_fn=collate_paired, num_workers=4
    )
    dev_loader = DataLoader(
        [dev_data[index]["student"] for index in range(len(dev_data))],
        batch_size=args.batch_size * 4, collate_fn=collate, num_workers=4,
    )
    ...
    teacher = CompactDependencyParser(
        teacher_config["encoder"], len(UPOS), len(relations),
        teacher_config.get("arc_size", 256), teacher_config.get("relation_size", 128),
        scale_biaffine=teacher_config.get("scale_biaffine", False),
    ).to(device)
    teacher.load_state_dict(torch.load(args.teacher / "model.pt", map_location=device, weights_only=True))
    teacher.eval().requires_grad_(False)
    student = CompactDependencyParser(
        args.student_encoder, len(UPOS), len(relations), args.arc_size, args.relation_size,
        scale_biaffine=args.scale_biaffine,
    ).to(device)

    encoder_parameters = list(student.encoder.parameters())
    encoder_ids = {id(parameter) for parameter in encoder_parameters}
    head_parameters = [parameter for parameter in student.parameters() if id(parameter) not in encoder_ids]
    optimizer = AdamW([
    ...
    metadata = {
        "encoder": args.student_encoder, "upos": UPOS, "relations": relations, "seed": args.seed,
        "teacher": str(args.teacher), "temperature": args.temperature,
        "gold_weight": args.gold_weight, "distill_weight": args.distill_weight,
        "arc_size": args.arc_size, "relation_size": args.relation_size,
        "scale_biaffine": args.scale_biaffine,
        "rule_data": str(args.rule_data) if args.rule_data else None,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for paired_batch in progress:
            student_batch = {key: value.to(device) for key, value in paired_batch["student"].items()}
            teacher_batch = {key: value.to(device) for key, value in paired_batch["teacher"].items()}
            optimizer.zero_grad(set_to_none=True)
            with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=device.type == "cuda"):
                with torch.no_grad():
                    teacher_output = teacher(
                        teacher_batch["input_ids"], teacher_batch["attention_mask"],
                        teacher_batch["word_starts"], teacher_batch["word_mask"], teacher_batch["heads"],
                    )
                student_output = student(
                    student_batch["input_ids"], student_batch["attention_mask"],
                    student_batch["word_starts"], student_batch["word_mask"], student_batch["heads"],
                )
                gold_loss, _ = losses(student_output, student_batch)
                valid = student_batch["word_mask"]
                kd_weights = student_batch["rule_weights"][valid]
                assert student_output.relation_logits is not None
                assert teacher_output.relation_logits is not None
                kd_upos = distribution_kl(
                    student_output.upos_logits[valid], teacher_output.upos_logits[valid], args.temperature, kd_weights
                )
                kd_arc = distribution_kl(
                    student_output.arc_logits[valid], teacher_output.arc_logits[valid], args.temperature, kd_weights
                )
                kd_relation = distribution_kl(
                    student_output.relation_logits[valid], teacher_output.relation_logits[valid], args.temperature, kd_weights
                )
            ...
            scaler.update()
            scheduler.step()
            progress.set_postfix(loss=f"{loss.item():.3f}", gold=f"{gold_loss.item():.3f}", kd=f"{kd_loss.item():.3f}")

        metrics = evaluate(student, dev_loader, device)
    ...
    student.load_state_dict(torch.load(args.output / "model.pt", map_location=device, weights_only=True))
    print(json.dumps({"test": evaluate(student, test_loader, device)}, indent=2))
    if rule_eval_loader is not None:
        print(json.dumps({"rule_eval": evaluate(student, rule_eval_loader, device)}, indent=2))
