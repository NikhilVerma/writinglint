...
from arguments import positive_float, positive_int, unit_interval
from data import UPOS, DependencyDataset, collate, relation_vocabulary
from decode import decode_tree
from model import CompactDependencyParser, ParserOutput
...
def losses(
    ...
    upos_loss = (
        F.cross_entropy(output.upos_logits[valid], batch["upos"][valid], reduction="none") * weights
    ).mean()
    arc_loss = (
        F.cross_entropy(output.arc_logits[valid], batch["heads"][valid], reduction="none") * weights
    ).mean()
    if output.relation_logits is None:
        raise RuntimeError("Relation logits are required when calculating supervised losses")
    relation_loss = (
        F.cross_entropy(output.relation_logits[valid], batch["relations"][valid], reduction="none")
        * weights
    ).mean()
...
def evaluate(
    ...
    for batch in loader:
        ...
        critical = valid & batch.get(
            "rule_weights", torch.ones_like(batch["heads"], dtype=torch.float)
        ).gt(1)
...
def main() -> None:
    ...
    parser.add_argument("--warmup-ratio", type=unit_interval, default=0.1)
    ...
    # The model architecture is derived from training data only. Development
    # and test datasets fail clearly if they contain an unseen relation.
    relations = relation_vocabulary([train_path])
    if not relations:
        raise ValueError(f"Training split contains no dependency relations: {train_path}")
    ...
    train_loader = DataLoader(
        train_data, batch_size=args.batch_size, shuffle=True, collate_fn=collate, num_workers=4
    )
    dev_loader = DataLoader(
        dev_data, batch_size=args.batch_size * 2, collate_fn=collate, num_workers=4
    )
    test_loader = DataLoader(
        test_data, batch_size=args.batch_size * 2, collate_fn=collate, num_workers=4
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in progress:
            ...
            progress.set_postfix(
                loss=f"{loss.item():.3f}", **{key: f"{value:.2f}" for key, value in parts.items()}
            )

        metrics = evaluate(model, dev_loader, device, dev_data.skipped_sentences)
    ...
    print(
        json.dumps(
            {"test": evaluate(model, test_loader, device, test_data.skipped_sentences)},
            indent=2,
        )
    )
