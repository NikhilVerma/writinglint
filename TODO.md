...
WritingLint must own its parser contract and must not ship or depend on company
IP from `nlpgraph`.

### 1. Extract the parser contract

- [x] Define `DepToken` and `ParsedSentence` in `writinglint-core`.
- [x] Remove all public and type-only imports from `nlpgraph`.
...
- [ ] Do not copy `nlpgraph` source, weights, tokenizer artifacts, vocabularies,
      configuration, or internal documentation.
...
Build a focused, agent-facing product on top of the Better Write engine. Its
promise is narrower and more memorable: stop AI-generated prose from shipping
with recognizable AI-slop constructions.

- [x] Split `sloplint` into its own consumer package, binary, tests, docs, model
      cache lifecycle, and exit-code contract. It is not a WritingLint CLI mode.
- [ ] Publish the dedicated `sloplint` package while keeping the rules and parser
...
- [x] Write the Sloplint model card and training/release runbook.
