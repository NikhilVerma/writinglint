  ...
  lint-writing:
    ...
    steps:
      ...
      - name: Lint high-confidence writing tells
        run: npx --yes slopsift@0.4.0 . --quiet --format github
```
  ...
  review-writing:
    ...
    steps:
      ...
      - name: Write a JSON Lines report
        run: npx --yes slopsift@0.4.0 . --level info --format json-lines --exit-zero > slopsift.jsonl
...
- name: Lint changed writing
  ...
  run: |
    ...
    if (( ${#files[@]} )); then
      npx --yes slopsift@0.4.0 "${files[@]}" --quiet --format github
    fi
```
