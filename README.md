...
## WritingLint and Sloplint

**WritingLint is the platform. Sloplint is a product built with it.**
...
Sloplint combines the WritingLint parser and AI-style rulepack into a narrower
experience: a zero-config CLI and editor integrations for catching recognizable
AI-writing habits before they ship.

```text
Sloplint CLI / Chrome / VS Code / website
                    │
                    ▼
...
That dependency direction is intentional. Sloplint may add focused defaults,
...
- `writinglint-parser-node` includes the weights for npm consumers, so both
  WritingLint and Sloplint work offline immediately after installation;
- Chrome and platform-specific VS Code packages copy that same verified bundle
  into the extension artifact;
- browser demos stream it from the `sloplint-models` R2 bucket through
  `sloplint.dev`, with direct immutable artifacts under
  `models.sloplint.dev/compact-int8-v1/`.
...
The Sloplint CLI is runnable from this checkout and its packed artifact is tested
in an isolated consumer. Its npm manifest remains private while the exact
unscoped package name is under npm review; do not install the unrelated
`slop-lint` package expecting this project.
