...
const installed = JSON.parse(await readFile(join(root, 'node_modules/sloplint/package.json'), 'utf8'));
...
const cli = join(root, 'node_modules/sloplint/dist/cli.js');
const result = spawnSync(process.execPath, [
  ...
  env: {
    ...
    SLOPLINT_MODEL_BASE_URL: 'http://127.0.0.1:9',
  },
});
...
console.log(`Verified sloplint@${installed.version} as an isolated npm consumer (${reports[0].messages.length} findings).`);
